...
const useReader = (fields: string[]) => {
  ...
  const [errorBiography, setErrorBiography] = useState<string | undefined>(
    undefined,
  );
  const [errorLinks, setErrorLinks] = useState<string | undefined>(undefined);
  const [errorDateOfBirth, setErrorDateOfBirth] = useState<string | undefined>(
    undefined,
  );
  ...
  const validateRequiredFields = (): boolean => {
    const validators: Record<string, () => boolean> = {
      ...
      biography: isBiographyValid,
      links: isLinksValid,
      dateOfBirth: isDateOfBirthValid,
    };

    return fields.every((field) => validators[field]?.() ?? false);
  };
