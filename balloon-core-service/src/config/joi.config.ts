...
export function getJoiConfig() {
  return Joi.object({
    ...
    RABBITMQ_RETRY_EXCHANGE: Joi.string().required(),
    RABBITMQ_RETRY_QUEUE: Joi.string().required(),
    RABBITMQ_RETRY_DELAY_MS: Joi.number().integer().positive().required(),
    RABBITMQ_MAX_RETRIES: Joi.number().integer().min(1).required(),
