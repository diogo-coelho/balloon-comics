...
describe('CreateReaderFromUserEventUseCase', () => {
  it('deve criar o leitor e registrar a versão do agregado quando o evento é novo e válido', async () => {
    ...
    const transaction = {
      ...
      consumerAggregateVersion: {
        getOrCreateForUpdate: jest
          .fn()
          .mockResolvedValue({ lastAppliedVersion: 0 }),
    ...
    await new CreateReaderFromUserEventUseCase(unitOfWork).execute(event);

    expect(transaction.processedEvents.tryMarkAsProcessed).toHaveBeenCalledWith(
      'event-1',
      'reader-sync',
    );
    expect(transaction.readers.upsert).toHaveBeenCalledWith(
      expect.objectContaining({ userId: 'user-1', email: 'ana@example.com' }),
    );
    expect(
      transaction.consumerAggregateVersion.updateVersion,
    ).toHaveBeenCalledWith('user-1', 'reader-sync', 1);
  });
  ...
  it('deve ignorar eventos duplicados ou já processados', async () => {
    ...
    await new CreateReaderFromUserEventUseCase({
      execute: jest.fn(async (fn) => fn(transaction)),
    }).execute(event);
