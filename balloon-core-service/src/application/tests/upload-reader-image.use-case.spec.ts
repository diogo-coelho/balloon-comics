...
describe("UploadReaderImageUseCase", () => {
  it("deve processar imagem, salvar e retornar a URL pública", async () => {
    ...
    const storage = {
      ...
      getPublicUrl: jest.fn().mockReturnValue("https://cdn.example.com/readers/avatar.webp"),
    };
    ...
    expect(readers.updateImageUrl).toHaveBeenCalledWith("user-id", "readers/avatar.webp");
    expect(result).toEqual({ id: "reader-id", imageUrl: "https://cdn.example.com/readers/avatar.webp" });
  });

  it("deve rejeitar quando o leitor não existe", async () => {
    await expect(
      new UploadReaderImageUseCase(
        { findByUserId: jest.fn().mockResolvedValue(null), updateImageUrl: jest.fn() } as any,
        { process: jest.fn() } as any,
        { uploadFile: jest.fn(), getPublicUrl: jest.fn() } as any,
      ).execute({ userId: "missing-user", file: { originalName: "avatar.png", mimeType: "image/png", size: 10, buffer: Buffer.from("x") } }),
