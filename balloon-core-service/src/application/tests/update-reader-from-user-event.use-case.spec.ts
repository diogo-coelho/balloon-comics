...
describe("UpdateReaderFromUserEventUseCase", () => {
  it("deve sincronizar dados do leitor e atualizar a versão do agregado", async () => {
    const event = {
      ...
      aggregateId: "user-1",
      aggregateVersion: 2,
      data: { userId: "user-1", username: "ana.souza", email: "ana.nova@example.com" },
    } as any;

    const transaction = {
      processedEvents: { tryMarkAsProcessed: jest.fn().mockResolvedValue(true) },
      consumerAggregateVersion: {
        getOrCreateForUpdate: jest.fn().mockResolvedValue({ lastAppliedVersion: 1 }),
        updateVersion: jest.fn().mockResolvedValue(undefined),
      },
      readers: { synchronizeUserData: jest.fn().mockResolvedValue(undefined) },
    };

    await new UpdateReaderFromUserEventUseCase({ execute: jest.fn(async (operation) => operation(transaction)) } as any).execute(event);
    ...
    expect(transaction.consumerAggregateVersion.updateVersion).toHaveBeenCalledWith("user-1", "reader-sync", 2);
  });
  ...
  it("deve rejeitar evento fora de ordem quando há lacuna na sequência", async () => {
    ...
    await expect(
      new UpdateReaderFromUserEventUseCase({ execute: jest.fn(async (operation) => operation(transaction)) } as any).execute(event),
