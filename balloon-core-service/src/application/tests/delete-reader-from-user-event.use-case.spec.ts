...
describe("DeleteReaderFromUserEventUseCase", () => {
  it("deve excluir o leitor e registrar a última versão processada", async () => {
    ...
    const transaction = {
      ...
      consumerAggregateVersion: {
        getOrCreateForUpdate: jest.fn().mockResolvedValue({ lastAppliedVersion: 2 }),
        updateVersion: jest.fn().mockResolvedValue(undefined),
      },
      readers: { deleteByUserId: jest.fn().mockResolvedValue(undefined) },
    };

    await new DeleteReaderFromUserEventUseCase({ execute: jest.fn(async (operation) => operation(transaction)) } as any).execute(event);

    expect(transaction.readers.deleteByUserId).toHaveBeenCalledWith("user-9");
    expect(transaction.consumerAggregateVersion.updateVersion).toHaveBeenCalledWith("user-9", "reader-sync", 3);
  });
  ...
  it("deve rejeitar evento fora de ordem durante exclusão", async () => {
    ...
    await expect(
      new DeleteReaderFromUserEventUseCase({ execute: jest.fn(async (operation) => operation(transaction)) } as any).execute(event),
