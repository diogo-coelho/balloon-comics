...
describe("RabbitMqRetryProvider", () => {
  it("deve publicar mensagem de retry no exchange com headers incrementados", async () => {
    ...
    const provider = new RabbitMqRetryProvider({ getOrThrow: jest.fn().mockReturnValue("balloon.retry") } as any);
  ...
  it("deve lançar erro quando a mensagem retry não encontrou rota válida", async () => {
    ...
    await expect(provider.publishRetry(message, 2)).rejects.toThrow("Mensagem de retry sem rota válida");
