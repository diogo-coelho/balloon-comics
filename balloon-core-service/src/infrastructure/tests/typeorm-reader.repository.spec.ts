...
describe("TypeOrmReaderRepository", () => {
  it("deve mapear, consultar e atualizar dados do leitor", async () => {
    ...
    const reader = Reader.create({ userId: "user-id", username: "ana", email: "ana@example.com" });

    await adapter.upsert(reader);
    const result = await adapter.findByUserId("user-id");
    await adapter.synchronizeUserData({ userId: "user-id", username: "ana.souza", email: "ana.souza@example.com" });
