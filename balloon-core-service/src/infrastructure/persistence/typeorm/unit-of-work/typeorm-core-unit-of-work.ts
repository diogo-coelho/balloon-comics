  ...
  execute<T>(
    ...
    return this.dataSource.transaction(async (manager) => {
      const transaction: CoreTransactionalPort = {
        readers: new TypeOrmReaderRepository(
          manager.getRepository(ReaderOrmEntity),
        ),
        processedEvents: new TypeOrmProcessedEventRepository(
          manager.getRepository(ProcessedEventOrmEntity),
        ),
        consumerAggregateVersion: new TypeOrmConsumerAggregateVersionRepository(
          manager.getRepository(ConsumerAggregateVersionOrmEntity),
        ),
        ageVerifications: new TypeOrmAgeVerificationRepository(
          manager.getRepository(AgeVerificationOrmEntity),
        ),
        socialMediaLinks: new TypeOrmSocialMediaLinkRepository(
          manager.getRepository(SocialMediaLinkOrmEntity),
        ),
      };
