...
describe('AgeVerification', () => {
  ...
  it('deve calcular maioridade como falsa antes do aniversário', () => {
    ...
    expect(AgeVerification.calculateLegalAge(dateOfBirth, referenceDate)).toBe(
      false,
    );
  });
  ...
  it('deve atualizar a data de nascimento e recalcular a maioridade', () => {
    ...
    expect(ageVerification.dateOfBirth).toEqual(
      new Date('2010-01-01T00:00:00.000Z'),
    );
    expect(ageVerification.hasLegalAge).toBe(false);
    expect(ageVerification.updatedAt.getTime()).toBeGreaterThan(
