...
describe("Entidade User", () => {
  ...
  it("deve atualizar dados de integração e incrementar a versão", () => {
    const user = new User("id", "ana", "ana@example.com", "hash", 1, new Date(0), new Date(0));
  ...
  it("não deve alterar a versão quando os dados permanecem iguais", () => {
    const user = new User("id", "ana", "ana@example.com", "hash", 3, new Date(0), new Date(0));

    expect(user.update({ username: "ana", email: "ana@example.com" })).toBe(false);
  ...
  it("deve alterar o hash da senha", () => {
    const user = User.create({ username: "ana", email: "ana@example.com", passwordHash: "old" });
