...
describe("LogoutUseCase", () => {
  it("deve limpar o refresh token do usuário", async () => {
    ...
    const users = { findById: jest.fn().mockResolvedValue(user), save: jest.fn() };
  ...
  it("deve ignorar logout de usuário inexistente", async () => {
    const users = { findById: jest.fn().mockResolvedValue(null), save: jest.fn() };

    await expect(new LogoutUseCase(users).execute("missing")).resolves.toBeUndefined();
